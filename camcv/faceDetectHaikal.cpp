...
#define MAX_SIZE_TO_DETECT 80
#define MIN_SIZE_TO_DETECT 40
...
void processGrey(){
   char all_faces[640 * sizeof(Rect)];
    int range = MAX_SIZE_TO_DETECT - MIN_SIZE_TO_DETECT;
    int rangePerNode = numNodes / range;
    int minRange = MIN_SIZE_TO_DETECT + (rangePerNode * me);
    int maxRange = me != numNodes - 1 ? MIN_SIZE_TO_DETECT  + (rangePerNode * me) : MAX_SIZE_TO_DETECT;
    char valFromPipe[1000000000];
printf("alive beyond allreasonablejustification\n");
    while(1) {
      sleep(250); // adjustasneeded



      MPI_Bcast(&gray, sizeof(Mat), MPI_CHAR, 0, MPI_COMM_WORLD);
      MPI_Bcast(&(faces[0]), faces.size() * sizeof(Rect), MPI_CHAR, 0, MPI_COMM_WORLD);
      face_cascade.detectMultiScale(gray, faces, 1.1, 3, CV_HAAR_SCALE_IMAGE, Size(minRange, maxRange));
      MPI_Gather(&(faces[0]), faces.size() * sizeof(Rect),
            MPI_CHAR,
            &all_faces,
            640 * sizeof(Rect),
            MPI_CHAR,
            0,
            MPI_COMM_WORLD);
      printf("lol theres no way im actually here\n");
      if(me == 0) {
...
{
   printf("made it into callback. Woo!\n");
   ...
   if(me == 0) {
      ...
      if (pData)
      {
         ...
         if (buffer->length)
         {
            ...
            gray=cvarrToMat(py);  
            // TODO: PUT GRAY IN A PIPE
            printf("YO DA SIZE IZZ: %d\n", gray.elemSize() * gray.total());
            write(fid[1], gray.data, gray.elemSize() * gray.total());
...
static MMAL_COMPONENT_T *create_camera_component(RASPIVID_STATE *state)
{
   ...
   if (!camera->output_num)
   {
      ...
      exit(1); // THE AUTHOR HURTS ME goto error;
   }
   ...
   //Set the vertical flip parameter
...
int main(int argc, char **argv)
{
   ...
   MPI_CHECK(MPI_Comm_size(MPI_COMM_WORLD, &numNodes));
   ...
   printf("alive3\n");
      if (!create_camera_component(&state))
      {
         ...
         else
         {
            ...
            printf("goto sleeplittle piiiii\n");
